...
def emit_thread_metadata(events, tid_map, threads):
    """Append the ``thread_name`` records (phase 'M') naming every lane."""
    lanes = sorted(LANE_NAMES.items())
    for handle, tid in sorted(tid_map.items(), key=lambda kv: kv[1]):
        lanes.append((tid, threads.get(handle, {}).get("name") or f"0x{handle:x}"))
    ...
    def add(self, name, ts, ph, tid, args=None):
        ...
        self._recs.append(
            (
                ts,
                len(self._recs),
                {"name": name, "ph": ph, "pid": 0, "tid": tid, "args": args or {}},
            )
        )
...
def export(tr):
    ...
    tl = _Timeline()
    tid_map = {}
    starts = [start for start, _, _ in tr.segments]
    ...
    def running_lane(ts):
        ...
        if i >= 0 and ts <= tr.segments[i][1]:
            return real_tid(tr.segments[i][2])
        return DEFAULT_LANE

    for start, end, tid in tr.segments:
        tl.span(start, end, real_tid(tid))

    for start, end in tr.isr_spans:
        tl.span(start, end, ISR_LANE)
    ...
    for ev in tr.events:
        ...
        for keyword, lane in KEYWORD_LANES:
            if keyword in name:
                # "semaphore_take_enter" reads better as "semaphore_take".
                tl.add(name.removesuffix("_enter"), ev.ts, "i", lane, _ints(fields))
                break
        else:
            thread_id = fields.get("thread_id")
            tid = running_lane(ev.ts) if thread_id is None else real_tid(thread_id)
            tl.add(name, ev.ts, "i", tid, _ints(fields))

    events = tl.emit()
    emit_thread_metadata(events, tid_map, tr.threads)
    return events
