...
MAIN_OUT = 1006 * US
WORKER_IN = 1007 * US
WORKER_SLEEP = 1008 * US
WORKER_OUT = 1009 * US
OPEN_END = 1010 * US  # last timestamp of the trace built by open_tail_trace().
...
def sample_trace(metadata_file, event_defs):
    ...
    "main" is scheduled in, is interrupted, takes a semaphore and emits a user
    event, then yields to "worker", which sleeps: one of every kind of record
    the exporter routes, in a schedule whose shape is easy to state.
    """
...
def test_every_lane_is_named(events):
...
def test_kernel_objects_reach_their_lane(events):
    ...
    assert by_name["semaphore_take"]["tid"] == ctf2perfetto.SEMAPHORE_LANE
    assert by_name["semaphore_take"]["args"]["id"] == SEM
...
def test_event_without_a_thread_id_lands_on_the_running_thread(events):
    # k_sleep_enter names no thread, so the lane it belongs to is the one that
    # was on the CPU when it was recorded.
...
def test_records_keep_the_time_they_happened(events):
    # Records are queued grouped by kind, so an exporter timestamping them in
    # queue order piles every instant up past the end of the trace.
    by_name = {e["name"]: e for e in events if e["ph"] == "i"}
    assert by_name["semaphore_take"]["ts"] == SEM_TAKE / US
    assert by_name["app_signal"]["ts"] == NAMED / US

    isr = [e for e in events if e["tid"] == ctf2perfetto.ISR_LANE and e["ph"] in ("B", "E")]
    ...
    main = [e for e in events if e["tid"] == MAIN_TID and e["ph"] in ("B", "E")]
...
def test_end_of_trace_keeps_the_running_thread(open_tail_events):
    # The final running span closes at the last recorded timestamp, not at the
    # previous context switch, and the open ISR span survives the same way.
    worker = [e for e in open_tail_events if e["tid"] == WORKER_TID and e["ph"] in ("B", "E")]
    assert [e["ts"] for e in worker] == [WORKER_IN / US, OPEN_END / US]

    isr_spans = (
        e
        for e in open_tail_events
        if e["tid"] == ctf2perfetto.ISR_LANE and e["ph"] in ("B", "E")
    )
    isr = [e for e in isr_spans]
...
def test_explicit_metadata_is_used(sample_trace, metadata_file, tmp_path):
    elsewhere = tmp_path / "elsewhere"
    elsewhere.mkdir()
    (elsewhere / "metadata").write_text(metadata_file.read_text())
    metadata_file.unlink()

    out = tmp_path / "out.json"
    argv = [str(sample_trace), "--metadata", str(elsewhere / "metadata"), "-o", str(out)]
    assert ctf2perfetto.main(argv) == 0
...
def test_undescribed_event_is_fatal(sample_trace, tmp_path):
    # An id the metadata does not describe cannot be skipped, so the decode
    # stops there; exporting what came before would hide the mismatch.
...
def test_trailing_partial_record_is_not_fatal(sample_trace, event_defs, tmp_path, kept):
    # A trace read while the application is still writing it ends mid record,
    # in its header or in its payload. Neither is a metadata mismatch.
